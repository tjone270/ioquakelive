...
static void FS_Startup(const char* gameName) {
	...
	Cmd_AddCommand("dir", FS_Which_f);   // [QL] "dir" replaces Q3's "which", same handler
...
On Windows, if pak00.pk3 is missing from fs_basepath/baseq3/, attempt to
locate the Steam installation of Quake Live (app ID 282440) and copy
pak00.pk3 from it. Also copies user configs (qzconfig.cfg, repconfig.cfg)
from the Steam user's profile folder to fs_homepath.
===================
*/
#ifdef _WIN32
#include <windows.h>
...
#define QL_PAK00_SIZE   962052238ULL
#define QL_PAK00_MD5    "75aaae7c836b9ebdb1d4cfd53ba1c958"

static qboolean FS_FileExistsOS(const char *path) {
	DWORD attr = GetFileAttributesA(path);
	return (attr != INVALID_FILE_ATTRIBUTES && !(attr & FILE_ATTRIBUTE_DIRECTORY));
}

static qboolean FS_DirExistsOS(const char *path) {
	DWORD attr = GetFileAttributesA(path);
	return (attr != INVALID_FILE_ATTRIBUTES && (attr & FILE_ATTRIBUTE_DIRECTORY));
}

/*
FS_FindSteamQLPath - Find Quake Live install path via Steam.
Checks Sys_SteamPath() first, then parses libraryfolders.vdf
for alternate library locations. Returns qtrue and fills qlPath if found.
*/
static qboolean FS_FindSteamQLPath(char *qlPath, int pathSize) {
	char *steamPath;
	char testPath[MAX_OSPATH];
	HKEY steamRegKey;
	char steamInstall[MAX_OSPATH];
	DWORD pathLen;
	FILE *vdf;
	char line[1024];

	// Method 1: Sys_SteamPath() - uses registry to find QL directly
	steamPath = Sys_SteamPath();
	if (steamPath[0]) {
		Com_sprintf(testPath, sizeof(testPath), "%s\\baseq3\\pak00.pk3", steamPath);
		if (FS_FileExistsOS(testPath)) {
			Q_strncpyz(qlPath, steamPath, pathSize);
			return qtrue;
		}
	}

	// Method 2: Find Steam install dir, parse libraryfolders.vdf
	steamInstall[0] = '\0';
	if (!RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Valve\\Steam", 0,
	                  KEY_QUERY_VALUE, &steamRegKey)) {
		pathLen = sizeof(steamInstall);
		if (RegQueryValueEx(steamRegKey, "SteamPath", NULL, NULL,
		                    (LPBYTE)steamInstall, &pathLen)) {
	...
	{
		char *s;
		for (s = steamInstall; *s; s++) {
			if (*s == '/') *s = '\\';
		}
	}

	// Parse libraryfolders.vdf for library paths that contain app 282440
	Com_sprintf(testPath, sizeof(testPath), "%s\\steamapps\\libraryfolders.vdf", steamInstall);
	vdf = fopen(testPath, "r");
	if (!vdf)
		return qfalse;

	{
		char currentPath[MAX_OSPATH];
		qboolean hasPath = qfalse;
		...
		while (fgets(line, sizeof(line), vdf)) {
			...
			if (!Q_stricmpn(p, "\"path\"", 6)) {
				...
				if (val) {
					...
					if (end) {
						*end = '\0';
						Q_strncpyz(currentPath, val, sizeof(currentPath));
						hasPath = qtrue;
					}
				}
			}
			// Look for "282440" (QL app ID) within an apps block
			else if (hasPath && strstr(p, "\"282440\"")) {
				char qlDir[MAX_OSPATH];
				fclose(vdf);
				// Build QL path: libraryPath/steamapps/common/Quake Live
				Com_sprintf(qlDir, sizeof(qlDir), "%s\\steamapps\\common\\Quake Live",
				            currentPath);
				// Convert forward slashes from VDF to backslashes
				{
					char *s;
					for (s = qlDir; *s; s++) {
						if (*s == '/') *s = '\\';
				...
				Com_sprintf(testPath, sizeof(testPath), "%s\\baseq3\\pak00.pk3", qlDir);
				if (FS_FileExistsOS(testPath)) {
					Q_strncpyz(qlPath, qlDir, pathSize);
					return qtrue;
				}
				return qfalse;
			}
			// Reset path when entering a new numbered block
			else if (*p == '}') {
				hasPath = qfalse;
				currentPath[0] = '\0';
			}
...
static qboolean FS_CopyFileWithProgress(const char *src, const char *dst,
	...
	// Get file size
	hFile = CreateFileA(src, GENERIC_READ, FILE_SHARE_READ, NULL,
	...
	while ((bytesRead = fread(buf, 1, sizeof(buf), fin)) > 0) {
		...
		if (fileSize.QuadPart > 0) {
			int pct = (int)((totalRead * 100ULL) / (ULONGLONG)fileSize.QuadPart);
			if (pct != lastPercent) {
				lastPercent = pct;
				Com_Printf("\r  %s: %d%%", displayName, pct);
			}
		}
	}
	Com_Printf("\r  %s: done       \n", displayName);
...
/*
FS_FindSteamUserFolder - Find a Steam64 ID folder under the QL install dir.
Steam stores per-user data in folders named with the user's Steam64 ID.
Returns qtrue and fills steamIdPath if exactly one numeric folder is found,
or picks the most recently modified one if multiple exist.
*/
static qboolean FS_FindSteamUserFolder(const char *qlPath, char *steamIdPath,
                                        int pathSize) {
	WIN32_FIND_DATAA fd;
	HANDLE hFind;
	char searchPath[MAX_OSPATH];
	char bestPath[MAX_OSPATH];
	FILETIME bestTime;
	int count = 0;

	Com_sprintf(searchPath, sizeof(searchPath), "%s\\*", qlPath);
	...
	do {
		// Steam64 IDs are long numeric strings (17 digits)
		if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
		    fd.cFileName[0] >= '0' && fd.cFileName[0] <= '9' &&
		    strlen(fd.cFileName) >= 10) {
			char cfgPath[MAX_OSPATH];
			// Check this folder has baseq3 with config files
			Com_sprintf(cfgPath, sizeof(cfgPath), "%s\\%s\\baseq3",
			            qlPath, fd.cFileName);
			if (FS_DirExistsOS(cfgPath)) {
				count++;
				if (CompareFileTime(&fd.ftLastWriteTime, &bestTime) > 0) {
					bestTime = fd.ftLastWriteTime;
					Com_sprintf(bestPath, sizeof(bestPath), "%s\\%s",
					            qlPath, fd.cFileName);
				}
			}
		}
	...
	if (count > 0) {
		Q_strncpyz(steamIdPath, bestPath, pathSize);
		return qtrue;
	}
	return qfalse;
}
...
static void FS_CopyFromSteam(void) {
	...
	Com_sprintf(pak00Dest, sizeof(pak00Dest), "%s\\%s\\pak00.pk3",
	            fs_basepath->string, BASEGAME_DIR);
	needPak = !FS_FileExistsOS(pak00Dest);

	if (!needPak)
		return;

	Com_Printf("\npak00.pk3 not found in %s\\%s\\\n", fs_basepath->string, BASEGAME_DIR);
	Com_Printf("Searching for Quake Live Steam installation...\n");

	if (!FS_FindSteamQLPath(qlPath, sizeof(qlPath))) {
		Com_Printf("Quake Live Steam installation not found.\n");
		return;
	}
	...
	Com_sprintf(message, sizeof(message),
		"Quake Live is installed at:\n  %s\n\n"
		"The following files will be copied:\n\n"
		"  pak00.pk3 (~917 MB)\n"
		"    From: %s\\baseq3\\\n"
		"    To:   %s\\%s\\\n",
		qlPath, qlPath, fs_basepath->string, BASEGAME_DIR);
	...
	if (hasConfigs) {
		...
		Q_strcat(message, sizeof(message),
		         va("    to: %s\\%s\\\n", fs_homepath->string, BASEGAME_DIR));
	}

	Q_strcat(message, sizeof(message), "\nProceed with copy?");

	if (Sys_Dialog(DT_YES_NO, message, "Quake Live - First Time Setup") != DR_YES) {
		Com_Printf("Copy cancelled by user.\n");
		return;
	}
	...
	{
		char dir[MAX_OSPATH];
		Com_sprintf(dir, sizeof(dir), "%s\\%s", fs_basepath->string, BASEGAME_DIR);
		Sys_Mkdir(dir);
	}

	// Copy pak00.pk3
	if (!FS_CopyFileWithProgress(pak00Src, pak00Dest, "pak00.pk3")) {
		Com_Printf("ERROR: Failed to copy pak00.pk3\n");
		Sys_Dialog(DT_ERROR,
			"Failed to copy pak00.pk3.\n\n"
			"Check that the source file is readable and the\n"
			"destination has enough free disk space (~917 MB).",
			"Copy Failed");
		return;
	}

	Com_Printf("pak00.pk3 copied successfully.\n");

	// Copy config files
	if (hasConfigs) {
		char homeDir[MAX_OSPATH];
		int i;
		Com_sprintf(homeDir, sizeof(homeDir), "%s\\%s",
		            fs_homepath->string, BASEGAME_DIR);
	...
	Com_Printf("\nQuake Live data setup complete.\n\n");
}
#endif /* _WIN32 */
...
/*
=====================
FS_VerifyPureGamecode

This code verifies that the currently loaded pak files used by
the game modules (ui and cgame) are among those allowed by the
server (as listed in fs_serverPaks[]), enforcing server purity.
=====================
*/
qboolean FS_VerifyPureGamecode(void) {
	// Return true by default unless a mismatch or missing pak is found
	if (fs_numServerPaks == 0) {
		return qtrue;  // No pure server paks to verify against
	}

	if (fs_uiBinChecksum == 0) {
		Com_DPrintf("no pak for ui module\n");
		return qfalse;
	}

	if (fs_cgameBinChecksum == 0) {
		Com_DPrintf("no pak for cgame module\n");
		return qfalse;
	}
	...
	// Traverse search paths (linked list of searchpath_t)
	for (searchpath_t* search = fs_searchpaths; search != NULL; search = search->next) {
		pack_t* pak = search->pack;

		if (pak && (pak->referenced & FS_UI_REF) && fs_numServerPaks > 0) {
			int checksum = pak->checksum;

			if (checksum != fs_serverPaks[0]) {  // Check if not first server pak
			...
			// Match against game module checksum
			if (fs_uiBinChecksum == checksum) {
				return qtrue;
			}
		}
	}

	Com_DPrintf("no pak found matching the game modules we have\n");
	return qfalse;
}
...
qboolean FS_ExtractGamecode(const char* module, char* outOSPath) {
	...
	for (search = fs_searchpaths; search != NULL; search = search->next) {
		...
		for (fileEntry = pak->hashTable[hash]; fileEntry; fileEntry = fileEntry->next) {
			...
			// Prepare to read from the .pak
			handleIn = FS_HandleForFile();
			fileHandleData_t* fhdIn = &fsh[handleIn];
			memset(fhdIn, 0, sizeof(fileHandleData_t));
			fhdIn->zipFile = qtrue;
			fhdIn->handleFiles.file.z = pak->handle;
			Q_strncpyz(fhdIn->name, filename, sizeof(fhdIn->name));

			unzSetCurrentFileInfoPosition(pak->handle, fileEntry->pos);
