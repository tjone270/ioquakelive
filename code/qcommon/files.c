...
static void FS_CheckPak00(void) {
    ...
    for (path = fs_searchpaths; path; path = path->next) {
        ...
        if (!Q_stricmpn(curpack->pakGamename, BASEGAME, MAX_OSPATH) && strlen(pakBasename) == 5 && !Q_stricmpn(pakBasename, "pak", 3) && pakBasename[3] >= '0' && pakBasename[3] <= '0' + NUM_ID_PAKS - 1) {
            if (curpack->checksum != pak_checksums[pakBasename[3] - '0']) {
                if (pakBasename[3] == '0') {
                    Com_Printf(
                        ...
                        "**************************************************\n"
                        "WARNING: " BASEGAME
                        "/pak00.pk3 is present but its checksum (%u)\n"
    ...
    if (!com_standalone->integer && (foundPak & ((1 << NUM_ID_PAKS) - 1)) != ((1 << NUM_ID_PAKS) - 1)) {
        ...
        Q_strcat(errorText, sizeof(errorText),
                 va("%s data files are missing. Please copy", PRODUCT_NAME));

        Q_strcat(errorText, sizeof(errorText),
                 va(" from the \"%s\" directory in your %s install or CD-ROM to:\n\n"
                    "%s%c%s%c\n\n",
                    BASEGAME, PRODUCT_NAME, installPath, PATH_SEP, BASEGAME, PATH_SEP));
        ...
        if (installHome) {
            ...
            Q_strcat(errorText, sizeof(errorText),
                     va("Also check that your %s executable is in "
